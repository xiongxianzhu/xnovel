...
Web 选区快捷操作使用现有 `selected_text` 字段和 `rewrite`、`expand`、`compress` 任务类型。用户先确认生成，再确认应用；客户端仅在内存保存选区边界、原正文与版本，按原边界组合替换结果，并通过现有应用接口提交完整候选正文。选区原版本必须与生成版本一致，服务端继续执行文档与版本校验；未保存、跨文档或过期候选不能绕过保护。快照不进入日志或额外持久化；有运行中任务或待处理候选时拒绝用新选区覆盖。
...
普通 AI 终态任务与结果保留 30 天；收藏和已确认结果保留，业务资料、不可变审计及有效 Skill 版本不自动清理。Desktop 使用当前设备上的用户凭据，与 Web 凭据不得隐式共享。

## 9. 摘要、分析与后台批次

结果用途明确区分 `manuscript`、`summary`、`analysis`。只有正文候选能调用原有应用接口；摘要要进入章节摘要审核，分析要经过结构校验和原句核对后成为待处理疑点。确认摘要不修改正式正文。

普通生成先调用 `/ai/context-preview`，显示文档版本、引用计数、已选 Skill 与前情资料、估算输入及可用预算；生成请求携带 `expected_document_version`。来源资料必须属于当前作品，摘要已确认，事实类型为 fact，且来源有效并位于当前章之前；推断、计划和过期资料不能进入该前情上下文。

中文估算对非 ASCII 字符按 2 tokens、ASCII 按约 4 字符/token 计入，再留出输出和安全余量；实际输入/输出用量以供应商返回为准，未返回时为空。没有可靠价格表，费用显示未知。

批次明确选择 1～1000 个正文；摘要和一致性均允许逐章执行。`use_recall` 仅包含所选范围中前两章原文，不读取未选择的全书。长章按预算分段；摘要可能再执行合并调用，预览估算不代表精确总账单。引用过大或无法在预算内安全执行时拒绝，不静默截断。

批次创建按所有者及 `request_id` 幂等；每用户一个批量项目在运行，与手动任务共享两调用上限。逐章开始时复核正文版本、来源顺序及模型可用性；改稿会标为过期。重启后暂停，运行项目失败，作者决定恢复未开始项或重试失败/过期项，成功项保持原结果。

一致性输出限制为结构化 issues，每条要有本章和另一所选来源章的原文证据。证据缺失、无法定位或输出结构非法均不能落为有效问题；解释仍是模型疑点，作者可解决或填写理由忽略。此检查不是穷尽式全书逻辑证明。

终态 SSE 缓存约 5 分钟后释放；普通历史定时清理。历史重试使用当前稿件和当前配置且由作者显式触发；不持久化选区原文，因此历史选区任务须回编辑器重新选择。完整正文、上下文和 Skill 不进入诊断日志。
