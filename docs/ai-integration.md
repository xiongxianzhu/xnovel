...
Web 的 Provider、凭据、四种协议适配器、最小上下文、流式任务、候选决策、Skill 快照与用量元数据已实现。服务进程启动时把遗留的排队/运行任务收敛为失败；PostgreSQL 使用按用户事务级 advisory lock 串行预占并发槽位，单进程同时保留轻量进程锁。

当前地址防护在保存与每次调用前解析 DNS、拒绝危险地址，并禁用环境代理和重定向；尚未把已解析 IP 固定到实际 socket peer，因此受控自定义域名仍需由部署方保证 DNS 可信。公开服务上线前应增加连接后 peer 校验或固定解析传输层。
