...
Web 登录后使用统一 AppShell：顶部导航承载品牌和账户入口，账户菜单进入个人资料、主动改密与偏好设置；侧边栏承载仪表盘、作品、AI 模型和系统设置。管理员额外看到用户、登录日志和操作日志入口。用户管理通过 `/api/admin/v1` 修改账户并在同一事务追加脱敏操作审计；停用用户会撤销现有会话但保留业务数据。登录日志只读聚合 `user_sessions` 与用户公开标识，操作日志只读聚合 `admin_audit_events` 与可用管理员用户名，不向浏览器返回密码哈希、令牌哈希或完整私密资料。前端根据内存中的用户角色过滤菜单，后端仍是权限最终边界。首次强制改密期间不渲染控制台侧边栏，路由只允许进入改密页面。
...
Web 作品列表按书名/独立作者署名查询，可与更新状态组合过滤后再分页。`/projects/:id/details` 为只读资料详情页，复用本人作品详情 API；原 `/projects/:id` 继续作为写作工作区。作者字段仅属于作品元数据，不绑定用户昵称。
...
正文保存和 AI 生成必须是两个独立操作。AI 输出先进入候选结果，用户确认后才能写入正文。两端 AI 任务在不含正文的上下文清单中固定生成时 `document_id` 和 `document_version`；应用时同时校验目标、生成版本和当前版本，不能用应用时读取的最新版本绕过过期检查。
...
- Web 头像和全局 Logo 使用随机存储键，服务端检查真实图片类型和解码尺寸；头像上限为 10 MiB，Logo 上限为 5 MiB。外链头像只接受 HTTPS，服务端不主动抓取。
- Web 品牌配置由 API 的 `site_settings` 单例持久化。管理员系统设置通过受保护接口更新名称、上传或移除 Logo；名称更新与管理员审计同事务提交。公开读取只返回站点名称、Logo URL 和注册开关；Web 使用同一个查询缓存驱动登录页、导航和浏览器标题，Desktop 不读取该配置。
...
Web 的独立管理路由通过生成客户端访问 API。回顾只读取已保存资料，不触发模型；检索直接在 PostgreSQL 限定作品查询，正文文本使用 `pg_trgm` 索引，不引入搜索服务或向量库。Web 查询缓存按登录账号隔离，退出或切换账号不会复用另一账号的私有结果。

API 拥有来源版本校验、资料确认、历史恢复、导入事务、交付统计和批次调度。资料失效在读取及任务上下文构建时动态判断；来源 UUID 保留用于提示删除与回退定位。故事时间与文档树顺序分别建模，未知或冲突信息不自动合并。

后台批次仍限定单 API 进程。批次和逐章状态先持久化，进程内按用户串行派发子任务；真实 Provider 调用使用既有 AI 任务并发槽位。浏览器只提交、轮询和发出控制指令，关页不会中止服务器调度。进程重启暂停批次，不自动重新调用中断项目。

正文历史与恢复留在各自存储边界：Web 使用 PostgreSQL，Desktop 使用 SQLite 和四个受限历史 IPC。`packages/text-diff` 只共享纯文本比较，不共享存储或权限。Desktop 仅补齐历史功能，其余长篇工作台仍由 Web 提供。

Web 记录列表、行操作、紧凑筛选和表单下拉统一使用 `src/shared/ui` 下的 `RecordTable`、`RowAction`、`FilterMenu` 和 `SelectField`。这些组件只负责展示、可访问交互和表单约束；查询分页、权限、保存与候选确认继续由页面及原有 API 负责。
