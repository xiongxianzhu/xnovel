...
Web 使用服务端可撤销的不透明访问/刷新令牌组合。短期访问令牌只保存在浏览器内存，受保护请求统一通过 `Authorization: Bearer <access_token>` 发送；长期刷新令牌只存在 `HttpOnly`、`Secure`、`SameSite=Lax` Cookie 中。页面重新加载后通过带来源与 CSRF 校验的刷新接口轮换刷新令牌并恢复访问令牌，失败时进入登录页。用户名、邮箱和手机号是统一登录框的三类标识；邮箱和手机号验证后才可用于登录。`site_settings` 是站点单例，动态控制公开注册和全局 Logo。用户语言、主题与私有 Skill 都以当前账户为边界。

当前已实现公开 `site-config` 与注册闭环。注册先读取单例并 fail-closed，再以 HMAC 摘要在 PostgreSQL 独立短事务递增两个固定窗口桶；随后校验密码并在新事务中锁定、复核注册开关，同时创建普通用户与默认偏好。用户名、邮箱或手机号冲突统一返回同一错误。注册不签发令牌，登录与会话仍由后续任务实现。
...
- Web 受保护接口统一使用 HTTP Bearer。管理接口先验证同一访问令牌，再根据用户角色授权，不使用独立管理请求头或浏览器端运维密钥。刷新 Cookie 只用于刷新接口，并要求可信来源、CSRF 校验和成功轮换。
